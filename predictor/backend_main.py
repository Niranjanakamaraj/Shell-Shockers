...
@app.get("/")
async def read_root():
    return {"message": "API is running. Go to /docs for the API documentation."}
...
async def predict_from_csv(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Separate ID column from features - ensure proper type handling
        ids = df['id'].astype(str)  # Convert IDs to strings for JSON compatibility
        ...
        # Format the output to include the 'id' - ensure proper type conversion
        results = []
        # Create dictionary with individual BlendPropertyN columns
        pred_dict = {f"BlendProperty{j+1}": float(pred[j]) for j in range(len(pred))}
        pred_dict["id"] = ids.iloc[i]
